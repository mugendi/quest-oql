...
class Helper {
    ...
    constructor(opts = {}) {
        ...
        this.dedupCols = opts.dedupCols !== undefined ? opts.dedupCols : false;
    ...
    __validate(name, obj, type = "select") {
        ...
        if (valid !== true) {
            ...
            let messages = valid.map(o => {
                let pattern = o.type == 'stringPattern' ? ' pattern matching' : '';
                let expected = `\n\t Expected${pattern}: ${o.expected} got ${o.actual} `
                return o.message + expected
            });
    ...
    async __parse_fields_arr(fields, type = 'aggregation') {
        ...
        for (let fieldArr of arrify(fields)) {
            ...
            this.__validate('field_name', { fieldName: fieldStr })
            ...
            if (op) {
                ...
                if (type == 'aggregation') {
                    ...
                    fieldStr = `${fieldStr} ${op} ${this.__escape_string(param)}`;
                }
            }
            ...
            if (param && type == 'aggregation') {
                fieldStr = `${fieldStr} as ${this.__escape_string(await this.__format_col_names(param,true))}`
            } else if (!param && this.forceAlias) {
                fieldStr = `${fieldStr} as ${this.__escape_string(await this.__format_col_names(fieldStr,true))}`
            }
